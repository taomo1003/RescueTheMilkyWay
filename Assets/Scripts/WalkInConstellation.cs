...
public class WalkInConstellation : MonoBehaviour {
	...
	void Update () {
        ...
        if (startGam && notInGame)
        {
            ...
            Debug.Log(current_constellation.Name);
        ...
        if (currentIndex >= totalLines) {
            ...
            star.transform.localPosition = new Vector3(226.0f,100.0f,0.1f);
            star.transform.localScale = new Vector3(2.0f, 2.0f, 0.5f);

            winText.text = "You made it!!!\nYou linked <b><color=red>" + constellation_Walk + "</color></b>.\nGo around to see what it is like.";
        ...
        if ((Input.GetKeyDown(KeyCode.R)||Input.GetAxis("Fire2") > 0 || CrossPlatformInputManager.GetButtonDown("B")) && !notInGame)
        {
            backToEarth();
        }
